...
// Get meeting url for chat
exports.getMeetingURL = async (req, res) => {
    try {
        const groupID = req?.query?.groupId
        const chat = await Chat.findById(groupID)
        if (chat?.meetingURL) res.status(200).json(chat?.meetingURL);
        const url = await Meeting.findOne({ isActive: false })
        if (!url) return res.status(404).json({ message: 'No URL available' })
        await Chat.findByIdAndUpdate(groupID, { $set: { meetingURL: url?.url } })
        await Meeting.findByIdAndUpdate(url._id, { $set: { isActive: true } })
